...
const LEDGER_KEY = "token-tugger-9000-ledger";
const SPIN_COST = 15;
...
let lastSpendTotal = 0;
let lastGrossWin = 0;
let lastSpendItems = [{ label: "Starter fee", cost: 0 }];
...
function loadLedger() {
  try {
    const saved = window.localStorage.getItem(LEDGER_KEY);
...
function saveLedger() {
  window.localStorage.setItem(LEDGER_KEY, JSON.stringify(ledger.slice(0, 8)));
}
...
function render() {
  ...
  grossWinEl.textContent = `${lastGrossWin} tokens`;
  spendTotalEl.textContent = `${lastSpendTotal} tokens`;
  renderSpendItems();
  renderLedger();
  spinButton.disabled = spinning || balance < SPIN_COST;
  shareButton.disabled = spinning;
  exportButton.disabled = spinning || ledger.length === 0;
...
function resetGame() {
  ...
  lastSpendTotal = 0;
  lastGrossWin = 0;
  lastSpendItems = [{ label: "Starter fee", cost: 0 }];
  ledger = [];
  saveBalance();
  saveLedger();
...
async function spin() {
  ...
  lastGrossWin = payout.tokens;

  const spendPlan = buildSpendPlan(payout);
  lastSpendItems = spendPlan.items;
  lastSpendTotal = spendPlan.total;

  const tax = payout.type === "win" ? spendPlan.total : spendPlan.total + randomInt(1, 3);
  ...
  saveBalance();
  addLedgerEntry(finalSymbols, payout, lastTax, delta);
...
function presentOutcome(reels, payout, tax) {
  ...
  taxLineEl.textContent = tax === 0
    ? "Miracle detected: no extra tax on this spin."
    : `${pick(taxLines)} Total extraction: ${tax} tokens.`;
  walletLineEl.textContent = pick(walletLines);
...
function renderLedger() {
  ...
  items.forEach((entry) => {
    ...
    net.className = `ledger-net ${entry.net >= 0 ? "positive" : "negative"}`;
...
function addLedgerEntry(reels, payout, tax, delta) {
  const reelLabels = reels.map((item) => item.label).join(", ");
  const summary = `${reelLabels}. Won ${payout.tokens}, spent ${tax}, machine verdict: ${payout.multiplier}.`;
